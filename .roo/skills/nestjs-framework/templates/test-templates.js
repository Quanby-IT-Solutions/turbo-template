...
const EXPECTED_IMPORTS = {
	...
	"service.template.ts": ["@nestjs/common", "@nestjs/cache-manager", "@nestjs/event-emitter"],
	"repository.template.ts": ["@nestjs/common", "@nestjs/typeorm", "typeorm"],
	"dto.template.ts": ["@nestjs/swagger", "class-validator", "class-transformer"],
	"entity.template.ts": ["typeorm", "class-transformer"],
	"module.template.ts": ["@nestjs/common", "@nestjs/typeorm"],
	"service.spec.template.ts": ["@nestjs/testing"],
}
...
function replacePlaceholders(content, entity) {
	...
	result = result.replace(
		/\{\{entity-display-name-plural\}\}/g,
		entity["entity-display-name-plural"]
	)
...
function validateNestJSConventions(content, templateName, entity) {
	...
	if (templateName.includes("module")) {
		if (
			!content.includes(`${entity.EntityName}sModule`) &&
			!content.includes(`${entity.EntityName}Module`)
		) {
			errors.push(
				`Module should be named ${entity.EntityName}Module or ${entity.EntityName}sModule`
			)
		}
	}
