    ...
    const loadAppointments = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          setStats({
            upcomingAppointments: upcoming.filter((apt) => {
              const date = new Date(apt.requestedDate)
              return date <= withinWeek
            }).length,
            pendingRequests: data.filter((apt) => apt.status === "PENDING").length,
            completedVisits: data.filter((apt) => apt.consultation).length,
          })
              ...
              <div className="px-4 lg:px-6">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Know exactly when to be ready and who you’re meeting with.</CardDescription>
                  </CardHeader>
              ...
              <div className="px-4 lg:px-6">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Meet history and self-check logs in one glance.</CardDescription>
                  </CardHeader>
