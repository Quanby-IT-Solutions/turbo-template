...
interface SelfCheckFaceScanProps {
  onResults?: (results: VitalSignsResults) => void | Promise<void>
}

export function SelfCheckFaceScan({ onResults }: SelfCheckFaceScanProps) {
  ...
  const handleRestartMeasurement = useCallback(() => {
    ...
    setMeasurementProgress(0)
    measurementStartTimeRef.current = null
    setShowResultsModal(false) // Close modal when restarting
    
    // If session exists and is terminated, we need to create a new one
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl flex items-center gap-2">
              ...
              Self Check - Vital Signs Measurement
            </CardTitle>
            <CardDescription>
      ...
      {/* Results Modal - Shows after scanning completes */}
