...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			...
			className={cn(
				"text-sidebar-foreground no-scrollbar flex min-h-0 flex-1 flex-col gap-0 overflow-auto group-data-[collapsible=icon]:overflow-hidden",
				className
			)}
			{...props}
		/>
...
const sidebarMenuButtonVariants = cva(
	"ring-sidebar-ring text-sidebar-foreground [&_*]:text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground hover:[&_*]:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground peer/menu-button group/menu-button [&>span]:text-sidebar-foreground flex w-full items-center gap-2 rounded-md p-2 text-left text-sm outline-hidden transition-all group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&_svg]:size-4 [&_svg]:shrink-0",
	{
		variants: {
			variant: {
...
const SidebarMenuButton = React.forwardRef<HTMLButtonElement, SidebarMenuButtonProps>(
	(
		{ isActive = false, variant, size, tooltip, className, asChild = false, children, ...props },
		ref
	) => {
		...
		const activeClasses = isActive
			? "bg-sidebar-primary text-sidebar-primary-foreground [&_*]:text-sidebar-primary-foreground font-medium"
			: ""
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
	return (
		<ul
			...
			className={cn(
				"border-sidebar-border mx-3.5 flex translate-x-px flex-col gap-1 border-l px-2.5 py-0.5 group-data-[collapsible=icon]:hidden",
				className
			)}
			{...props}
		/>
...
function SidebarMenuSubButton({
	...
	return (
		<Comp
			...
			style={{ ...activeStyle, ...propsStyle } as React.CSSProperties}
			className={cn(
				"text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground [&>svg]:text-sidebar-accent-foreground [&>span]:text-sidebar-foreground flex h-7 -translate-x-px items-center gap-2 rounded-md px-2 outline-hidden group-data-[collapsible=icon]:hidden focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[size=md]:text-sm data-[size=sm]:text-xs [&>svg]:size-4 [&>svg]:shrink-0",
				activeClasses,
				className,
				propsClassName
			)}
