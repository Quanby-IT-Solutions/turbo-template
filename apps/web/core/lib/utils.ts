...
export const getAppUrl = cache(() => {
	return env.NEXT_PUBLIC_APP_URL ?? process.env.NEXT_PUBLIC_APP_URL ?? DEFAULT_APP_URL
})
...
export const getApiUrl = cache(() => {
	...
	const version =
		env.NEXT_PUBLIC_API_VERSION ?? process.env.NEXT_PUBLIC_API_VERSION ?? DEFAULT_API_VERSION

	return `${baseUrl.replace(/\/$/, "")}/${version.replace(/^\//, "")}`
})
