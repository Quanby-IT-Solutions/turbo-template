...
/**
 * The timestamp fields accept either shape because the same schema parses rows
 * straight out of Drizzle (`Date`) and JSON off the wire (ISO string). Both
 * arms are exercised here — `CreateTicketSchema` picks neither field, so
 * nothing else in the suite ever runs the transforms.
 */
